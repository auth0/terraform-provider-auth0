...
- `access_token` (Block List, Max: 1) Configuration for the access tokens issued for this resource server. Remove the block to clear the configuration on the API. (EA only) (see [below for nested schema](#nestedblock--access_token))
...
<a id="nestedblock--access_token"></a>
### Nested Schema for `access_token`

Optional:

- `claims_mapping` (Block List, Max: 1) Configuration for mapping claims into the access tokens issued for this resource server. (EA only) (see [below for nested schema](#nestedblock--access_token--claims_mapping))

<a id="nestedblock--access_token--claims_mapping"></a>
### Nested Schema for `access_token.claims_mapping`

Optional:

- `custom_claims` (Block List, Max: 20) Custom claims to include in the access tokens issued for this resource server. Maximum of 20 claims. Setting an empty list clears the custom claims on the API. (EA only) (see [below for nested schema](#nestedblock--access_token--claims_mapping--custom_claims))

<a id="nestedblock--access_token--claims_mapping--custom_claims"></a>
### Nested Schema for `access_token.claims_mapping.custom_claims`

Required:

- `expression` (String) Expression used to resolve the claim value, given as a dot-path read from the request context (for example `anonymous_session.metadata.country`).
- `name` (String) Name of the claim to emit in the access token. Reserved OIDC/JWT claim names are not allowed.
...
- `anonymous_user` (Block List, Max: 1) Anonymous user authorization policies for the resource server. (EA only) (see [below for nested schema](#nestedblock--subject_type_authorization--anonymous_user))
...
<a id="nestedblock--subject_type_authorization--anonymous_user"></a>
### Nested Schema for `subject_type_authorization.anonymous_user`

Optional:

- `policy` (String) Anonymous user flows policy. One of `deny_all`, `require_client_grant`. Defaults to `deny_all` (EA only)
