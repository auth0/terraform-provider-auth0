// nested.go — schema attribute-types and expand/flatten helpers for the
// nested objects on auth0_client added in Wave 2 (jwt_configuration,
// refresh_token, oidc_logout, encryption_key, default_organization,
// native_social_login, session_transfer, mobile, signed_request_object,
// token_exchange, my_organization_configuration, express_configuration).
//
// Heavily polymorphic groups (`addons`, `client_authentication_methods`) are
// exposed as JSON-string attributes — see resource.go.
package client
