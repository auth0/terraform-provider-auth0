...
<a id="nestedblock--token_vault_privileged_access"></a>
### Nested Schema for `token_vault_privileged_access`

Optional:

- `credentials` (Block List, Max: 2) Credentials attached to the privileged worker. Required whenever `token_vault_privileged_access` is being written; sending an empty list detaches every credential and disables the worker. (EA only) (see [below for nested schema](#nestedblock--token_vault_privileged_access--credentials))
- `grants` (Block List, Max: 5) Pins the connections, and the scopes within them, that the privileged worker may request tokens for. Maximum of 5 grants, with a maximum of 20 scopes in total across all of them. Required whenever `token_vault_privileged_access` is being written. (EA only) (see [below for nested schema](#nestedblock--token_vault_privileged_access--grants))
- `ip_allowlist` (Set of String) List of permitted IPv4 or IPv6 addresses, or CIDR ranges, from which the privileged worker may request tokens. Maximum of 10 entries. Required whenever `token_vault_privileged_access` is being written. (EA only)
...
- `credential_type` (String) Credential type. Supported types: `public_key`. (EA only)
- `pem` (String, Sensitive) PEM-formatted public key. Must be JSON escaped. (EA only)

Read-Only:

- `id` (String) The ID of the client credential. (EA only)


<a id="nestedblock--token_vault_privileged_access--grants"></a>
### Nested Schema for `token_vault_privileged_access.grants`

Required:

- `connection` (String) Name of the connection the grant applies to. (EA only)
- `scopes` (Set of String) Scopes the privileged worker may request on the connection. Must be non-empty and unique. (EA only)
