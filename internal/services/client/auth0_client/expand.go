...
func applyCreateOptional(ctx context.Context, plan *model, body *mgmt.CreateClientRequestContent, diags *diag.Diagnostics) {
	...
	// enums set directly as pointers
	if v := plan.ThirdPartySecurityMode; !v.IsNull() && !v.IsUnknown() {
		enum, err := mgmt.NewClientThirdPartySecurityModeEnumFromString(v.ValueString())
		if err != nil {
			diags.AddAttributeError(path.Root("third_party_security_mode"), "Invalid third_party_security_mode", err.Error())
			return
		}
		body.ThirdPartySecurityMode = &enum
	}
	if v := plan.RedirectionPolicy; !v.IsNull() && !v.IsUnknown() {
		enum, err := mgmt.NewClientRedirectionPolicyEnumFromString(v.ValueString())
		if err != nil {
			diags.AddAttributeError(path.Root("redirection_policy"), "Invalid redirection_policy", err.Error())
			return
		}
		body.RedirectionPolicy = &enum
	}
