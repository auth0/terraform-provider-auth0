...
- `token_vault_privileged_access` (Block List, Max: 1) Configures the client as a Token Vault privileged worker, allowing it to request Token Vault tokens on behalf of other users. This is an Early Access feature and must be enabled for your tenant. (see [below for nested schema](#nestedblock--token_vault_privileged_access))
...
- `connection` (String) Name of the connection the grant applies to. The connection does not need to exist when the grant is configured; it is validated at runtime.
- `scopes` (Set of String) Scopes the privileged worker may request on the connection.
